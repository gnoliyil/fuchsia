...
static void riscv64_page_fault_handler(long cause, struct iframe_t* frame) {
  ...
  pf_flags |= !(frame->sstatus & RISCV64_CSR_SSTATUS_PP) ? VMM_PF_FLAG_USER : 0;
...
static void riscv64_illegal_instruction_handler(long cause, struct iframe_t* frame) {
  ...
  if ((frame->sstatus & RISCV64_CSR_SSTATUS_FS) != RISCV64_CSR_SSTATUS_FS_OFF) {
    try_dispatch_user_data_fault_exception(ZX_EXCP_UNDEFINED_INSTRUCTION, frame);
  }
  // Otherwise we just try to enable the FPU.
  frame->sstatus |= RISCV64_CSR_SSTATUS_FS_INITIAL;
}
...
extern "C" void riscv64_exception_handler(long cause, struct iframe_t* frame) {
  LTRACEF("hart %u cause %s epc %#lx status %#lx\n", arch_curr_cpu_num(), cause_to_string(cause),
          frame->regs.pc, frame->sstatus);
