...
// TODO: move to lib/arch accessors
// TODO: add driver logic to probe extensions first
...
// Note: it seems ambiguous whether or not a2-a7 are trashed in the call, but the
...
static inline void sbi_set_timer(uint64_t stime_value) { sbi_call(SBI_SET_TIMER, stime_value); }
...
static inline sbiret sbi_remote_sfence_vma(const unsigned long *hart_mask,
                                           unsigned long hart_mask_base, unsigned long start,
                                           unsigned long size) {
