...
// TODO(fxbug.dev/91187): Pick a load address through a sort of allocator
...
#ifdef __aarch64__
  // TODO(fxbug.dev/135733): There is no upper address space in EL2+ by
  // default, so drop down to EL1 and proceed with the test.
  if (auto el = arch::ArmCurrentEl::Read().el(); el > 1) {
    printf(
        "TODO(fxbug.dev/135733): %s test is only supported for EL1 (current EL is %lu); skipping it\n",
        kTestName, el);
    return 0;
  }
#endif
