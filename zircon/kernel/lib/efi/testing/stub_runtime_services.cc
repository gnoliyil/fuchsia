...
efi_status StubRuntimeServices::GetVariable(char16_t* var_name, efi_guid* vendor_guid,
  ...
  auto it = std::find_if(vars_->begin(), vars_->end(),
                         [&](const std::pair<VariableName, VariableValue>& it) {
                           return it.first == VariableName{var_name, *vendor_guid};
                         });
  if (it == vars_->end())
...
efi_status StubRuntimeServices::GetNextVariableName(size_t* var_name_size, char16_t* var_name,
  ...
  std::u16string var_name_str(var_name, str_len_safe);
  auto it = var_it_;
  if (var_name_str.compare(u"") == 0) {
...
bool operator<(const StubRuntimeServices::VariableName& l,
  ...
  auto cmp = l.var_name.compare(r.var_name);
  if (cmp == 0) {
...
bool operator==(const StubRuntimeServices::VariableName& l,
                const StubRuntimeServices::VariableName& r) {
  if (l.var_name.compare(r.var_name) == 0) {
    return l.guid == r.guid;
  }
  return false;
}
