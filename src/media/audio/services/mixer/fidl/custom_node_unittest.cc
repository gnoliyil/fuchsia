...
class CustomNodeTest : public ::testing::Test {
 protected:
  ...
  auto MakeProcessorConfigBuilder(uint64_t block_size_frames = 1, uint64_t latency_frames = 0) {
    ...
    builder.inputs(MakeInputs(MakeInputConfigBuilder()
                                  .buffer(MakeBuffer())
...
TEST_F(CustomNodeTest, CreateDeleteEdge) {
  ...
  Node::PrepareToDelete(ctx, custom_node);
  ...
  auto custom_node = CustomNode::Create({
      ...
      .config =
          MakeProcessorConfigBuilder()
              .inputs(MakeInputs(
                  MakeInputConfigBuilder().format(kFormat.ToLegacyMediastreamsWireFidl()).Build()))
              .Build(),
  });
}
...
TEST_F(CustomNodeTest, CreateFailsMissingOutputBuffer) {
  auto custom_node = CustomNode::Create({
      ...
      .config =
          MakeProcessorConfigBuilder()
              .outputs(MakeOutputs(
                  MakeOutputConfigBuilder().format(kFormat.ToLegacyMediastreamsWireFidl()).Build()))
              .Build(),
  });
}
