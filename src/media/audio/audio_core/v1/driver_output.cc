...
std::optional<AudioOutput::FrameSpan> DriverOutput::StartMixJob(zx::time ref_time) {
  ...
  FX_DCHECK(driver()->driver_transfer_frames());
  uint32_t driver_transfer_frames = *driver()->driver_transfer_frames();
  ...
  int64_t output_frames_transmitted = output_frames_consumed - driver_transfer_frames;
...
void DriverOutput::OnDriverStartComplete() {
  ...
  //
  // TODO(https://fxbug.dev/42115795): an intermediate buffer probably need not be as large as the
  ...
  //
  // TODO(https://fxbug.dev/42135216): Keep reference clocks in sync with mono time under test.
