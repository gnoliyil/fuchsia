...
zx::result<> FilesystemMounter::MountLegacyFilesystem(
  ...
  // Errors are ignored and are benign in these tests.
  ...
  if (DiskFormatComponentUrl(format).empty()) {
    ...
    const std::string device_path =
        GetTopologicalPath(
            fidl::UnownedClientEnd<fuchsia_device::Controller>(block_device.channel().borrow()))
            .value_or(std::string{});
    fidl::ClientEnd device = component::Clone(block_device, component::AssumeProtocolComposesNode)
                                 .value_or(fidl::ClientEnd<fuchsia_hardware_block::Block>{});
