...
impl<H: Hasher> MerkleTree<H> {
    ...
    ///   hashes_per_block (`block size` \ H::Digest::DIGEST_LEN`), rounded up to the nearest
    ///   integer.
    pub fn from_levels(levels: Vec<Vec<H::Digest>>) -> MerkleTree<H> {
        MerkleTree { levels }
    }
    ...
    pub fn from_reader(
        ...
        hasher: impl HasherTrait<H>,
    ) -> Result<MerkleTree<H>, io::Error> {
...
impl<H: Hasher> AsRef<[Vec<H::Digest>]> for MerkleTree<H> {
    fn as_ref(&self) -> &[Vec<H::Digest>] {
        &self.levels[..]
    }
}
...
mod tests {
    ...
    use hex::FromHex;
    use mundane::hash::{Digest, Sha256, Sha512};

    impl<H: Hasher> MerkleTree<H> {
        /// Given the index of a block of data, lookup its hash.
        fn leaf_hash(&self, block: usize) -> &H::Digest {
            &self.levels[0][block]
        }
    }
    ...
    fn test_single_full_hash_block_sha256() {
        ...
        let tree: MerkleTree<Sha256> = MerkleTree::from_levels(vec![leafs, vec![root]]);
        assert_eq!(tree.root().bytes(), root_bytes);
    ...
    fn test_single_full_hash_block_sha512() {
        ...
        let root = hasher.hash_hashes(&leafs);
        let root_bytes = root.bytes();
        let tree: MerkleTree<Sha512> = MerkleTree::from_levels(vec![leafs, vec![root]]);
        assert_eq!(tree.root().bytes(), root_bytes);
        for (i, leaf) in expected_leafs.iter().enumerate().take(hashes_per_block) {
            assert_eq!(tree.leaf_hash(i), leaf);
        }
