...
TEST(TestQuery, Sram) {
  ...
  if ((identity.chip_model == kMsdVsiVipDevice8000 &&
       identity.customer_id == MAGMA_VSI_VIP_NELSON_CUSTOMER_ID) ||
      (identity.chip_model == kMsdVsiVipDevice9000 &&
       identity.customer_id == MAGMA_VSI_VIP_A5_CUSTOMER_ID)) {
