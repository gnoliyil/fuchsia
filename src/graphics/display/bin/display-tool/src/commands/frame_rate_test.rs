...
// `rows` must be even.
//
// TODO(fxbug.dev/134912): The precondition can be relaxed to "either `rows` or
// `cols` must be even".
fn get_position_in_grid_for_frame(frame_num: u64, rows: u32, cols: u32) -> Point2D<u32> {
    ...
    // of rows is even.
    //
    // TODO(fxbug.dev/134912): Actually it works as long as either `rows` or
...
pub async fn run(
    ...
    // TODO(fxbug.dev/135194): Rather than manually checking the display engine
