...
void magma_connection_release(magma_connection_t connection) {
  // TODO(https://fxbug.dev/12731): close the connection
...
magma_status_t magma_poll(magma_poll_item_t* items, uint32_t count, uint64_t timeout_ns) {
  ...
  if (count == 1 && items[0].type == MAGMA_POLL_TYPE_SEMAPHORE &&
    ...
    items[0].result = 0;
    // TODO(https://fxbug.dev/49103): change WaitNoReset to take ns
  ...
  // TODO(https://fxbug.dev/49103): change PlatformPort::Wait to take ns
  uint64_t key;
