...
struct AbiPtr {
 public:
  ...
  constexpr explicit operator bool() const { return ComparisonValue() != 0; }

#if __cpp_impl_three_way_comparison >= 201907L

  constexpr int operator<=>(const AbiPtr& other) const noexcept {
  ...
  constexpr bool operator!=(const AbiPtr& other) const noexcept {
    return ComparisonValue() != other.ComparisonValue();
  }
  ...
  constexpr AbiPtr operator+(size_type n) const noexcept {
  ...
  constexpr size_type operator-(const AbiPtr& other) const noexcept {
    return static_cast<size_type>(storage_ - other.storage_) / kScale;
  }

  constexpr AbiPtr& operator+=(size_type n) noexcept {
    *this = *this + n;
    return *this;
  }
