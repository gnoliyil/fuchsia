...
async fn test_night_mode() {
    let instance = common::NightModeTest::create_realm().await.expect("setting up test realm");
    let initial_value = NightModeInfo { night_mode_enabled: None };
    let changed_value = NightModeInfo { night_mode_enabled: Some(true) };
    let night_mode_service = common::NightModeTest::connect_to_night_mode_marker(&instance);
