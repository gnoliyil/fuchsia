...
#[derive(Clone)]
pub struct Delegate<P: Payload + 'static, A: Address + 'static> {
    messenger_action_tx: MessengerActionSender<P, A>,
}

impl<P: Payload + 'static, A: Address + 'static> Delegate<P, A> {
    pub(super) fn new(action_tx: MessengerActionSender<P, A>) -> Delegate<P, A> {
        Delegate { messenger_action_tx: action_tx }
    }

    /// Returns a builder for constructing a new messenger.
    pub(crate) fn messenger_builder(&self, messenger_type: MessengerType<P, A>) -> Builder<P, A> {
