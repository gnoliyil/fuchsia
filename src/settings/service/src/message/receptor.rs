...
use crate::message::base::{MessageEvent, Payload, Signature, Status};
...
type EventReceiver<P> = UnboundedReceiver<MessageEvent<P>>;
...
pub struct Receptor<P: Payload + 'static> {
    signature: Signature,
    event_rx: EventReceiver<P>,
    // Fuse to be triggered when all receptors go out of scope.
    _fuse: ActionFuseHandle,
}

impl<P: Payload + 'static> Stream for Receptor<P> {
    type Item = MessageEvent<P>;
...
impl<P: Payload + 'static> Receptor<P> {
    pub(super) fn new(
        signature: Signature,
        event_rx: EventReceiver<P>,
    ...
    pub(crate) async fn next_payload(&mut self) -> Result<(P, MessageClient<P>), Error> {
    ...
    pub(crate) async fn next_of<T: TryFrom<P>>(&mut self) -> Result<(T, MessageClient<P>), Error>
    where
...
pub(crate) fn extract_payload<P: Payload + 'static>(event: Option<MessageEvent<P>>) -> Option<P> {
