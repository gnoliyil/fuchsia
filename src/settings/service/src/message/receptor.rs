...
use crate::message::base::{Address, MessageEvent, Payload, Signature, Status};
...
impl<P: Payload + 'static, A: Address + 'static> Receptor<P, A> {
    ...
    pub(crate) async fn next_payload(&mut self) -> Result<(P, MessageClient<P, A>), Error> {
    ...
    pub(crate) async fn next_of<T: TryFrom<P>>(&mut self) -> Result<(T, MessageClient<P, A>), Error>
    where
...
pub(crate) fn extract_payload<P: Payload + 'static, A: Address + 'static>(
