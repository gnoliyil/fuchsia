...
// Check if current class has |vk_debug_utils_message_collector()| function (e.g.
// TestWithVkValidationLayer or its derived class) at compile-time.
#define CHECK_IS_TEST_WITH_VK_VALIDATION_LAYER_DEFAULT()                                          \
  static_assert(                                                                                  \
      std::is_same<::escher::test::impl::VkDebugUtilsMessageCollector,                            \
                   std::remove_reference_t<decltype(vk_debug_utils_message_collector())>>::value, \
      "This macro can be only used on classes with |VkDebugUtilsMessageCollector| element "       \
      "named |debug_utils_message_collector_|.")

// Helper macros used to generate EXPECT_... macros for validation layer check.
#define EXPECT_VULKAN_VALIDATION_REPORT_PRED_(severity, types, display_flags, pred, pred_op, \
                                              num_threshold)                                 \
  do {                                                                                       \
    GTEST_AMBIGUOUS_ELSE_BLOCKER_;                                                           \
    CHECK_IS_TEST_WITH_VK_VALIDATION_LAYER_DEFAULT();                                        \
    if (!vk_debug_utils_message_collector().PrintDebugUtilsMessagesOnFalsePredicate(         \
            severity, types, num_threshold, pred, __FILE__, __LINE__))                       \
...
#define ASSERT_VULKAN_VALIDATION_REPORT_PRED_(severity, types, display_flags, pred, pred_op,     \
                                              num_threshold)                                     \
  do {                                                                                           \
    GTEST_AMBIGUOUS_ELSE_BLOCKER_;                                                               \
    CHECK_IS_TEST_WITH_VK_VALIDATION_LAYER_DEFAULT();                                            \
...
#define CHECK_VULKAN_VALIDATION_ERRORS_PRED_(fatal, pred, pred_op, num_threshold)                \
  fatal##_VULKAN_VALIDATION_REPORT_PRED_(vk::DebugUtilsMessageSeverityFlagBitsEXT::eError,       \
                                         vk::DebugUtilsMessageTypeFlagBitsEXT::eValidation |     \
                                             vk::DebugUtilsMessageTypeFlagBitsEXT::ePerformance, \
                                         "ERROR", pred, pred_op, num_threshold)
#define CHECK_VULKAN_VALIDATION_WARNINGS_PRED_(fatal, pred, pred_op, num_threshold)          \
  fatal##_VULKAN_VALIDATION_REPORT_PRED_(vk::DebugUtilsMessageSeverityFlagBitsEXT::eWarning, \
                                         vk::DebugUtilsMessageTypeFlagBitsEXT::eValidation,  \
                                         "WARNING", pred, pred_op, num_threshold)
