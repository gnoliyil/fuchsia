...
void PaperDrawCallFactory::EnqueueDrawCalls(const PaperShapeCacheEntry& cache_entry,
  ...
  {
    Hasher h;
    // TODO(https://fxbug.dev/42151868): add this back in some way, with a more abstract pipeline
  ...
  if (cast_shadows) {
    ...
    // TODO(https://fxbug.dev/42151868): revisit sort key... we expect that a subsequent CL will add a
...
PaperDrawCallFactory::SortKey PaperDrawCallFactory::SortKey::NewOpaque(Hash pipeline_hash,
  ...
  // TODO(https://fxbug.dev/42151868): We currently don't have multiple pipelines used in the opaque
