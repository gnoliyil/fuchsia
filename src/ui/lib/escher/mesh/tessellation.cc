...
IndexedTriangleMesh3d<vec2> NewCubeIndexedTriangleMesh(const MeshSpec& spec) {
  ...
  // TODO(https://fxbug.dev/42152601): Add separate box mesh type with split verts and proper uv coords. Since
...
MeshPtr NewRingMesh(MeshBuilderFactory* factory, BatchGpuUploader* gpu_uploader,
  ...
  for (uint32_t i = 0; i < outer_vertex_count; ++i) {
    ...
    if (vertex_p.uv) {
      ...
      // TODO(https://fxbug.dev/42151401): once we can specify a SamplerAddressMode of eClampToEdge,
...
MeshPtr NewSphereMesh(MeshBuilderFactory* factory, BatchGpuUploader* gpu_uploader,
  ...
  // TODO(https://fxbug.dev/42152845): this is a hack to ease implementation.  We don't currently
  ...
  while (subdivisions-- > 0) {
    ...
    // TODO(https://fxbug.dev/42152845): see comment in header file... this approach is broken, but
