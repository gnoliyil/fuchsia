...
bool BufferPool::CreateBuffers(size_t count, BufferPool::Environment* environment,
  ...
  auto collection_closer = fit::defer([environment, display_collection_id]() {
    if ((*environment->display_coordinator)->ReleaseBufferCollection(display_collection_id) !=
        ZX_OK) {
  ...
  auto image_constraints_info = escher::GetDefaultImageConstraintsInfo(create_info);
  auto constraints_result = environment->vk_device.setBufferCollectionImageConstraintsFUCHSIA(
      import_result.value, image_constraints_info.image_constraints,
      environment->escher->device()->dispatch_loader());
