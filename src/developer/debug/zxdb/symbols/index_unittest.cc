...
// Test for a single .dwo file generated by "debug fission". This file contains the isolated symbols
// for just that compilation unit. The ELF sections have different names than in non-DWO files.
TEST(Index, IndexDwo) {
  std::string fission_data_dir = TestSymbolModule::GetTestDataDir() + "fission/";
  TestSymbolModule setup(fission_data_dir + "main.dwo", "main.dwo");
  ASSERT_TRUE(setup.Init(fission_data_dir, false).ok());
  ...
  const char kExpected[] = R"(  Types:
    ...
    int: 0x32
    long int: 0x39
    long unsigned int: 0x1a
    short int: 0x2e
    short unsigned int: 0x26
    signed char: 0x2a
    unsigned char: 0x22
    unsigned int: 0x1e
  Functions:
...
// Tests that the main binary for "debug fission" symbols finds the references to the .dwo files.
// Compare to the ModuleSymbols.IndexFission test which constructs the index for the merged symbols
// for this binary.
TEST(Index, Fission) {
  std::string fission_data_dir = TestSymbolModule::GetTestDataDir() + "fission/";
  ...
  // There should be two .dwo refs.
  ASSERT_EQ(2u, index.dwo_refs().size());
  EXPECT_EQ("other.dwo", index.dwo_refs()[0].dwo_name);
  EXPECT_EQ("main.dwo", index.dwo_refs()[1].dwo_name);
}
