...
mod test {
    ...
    impl AsyncRead for TestTransport {
        fn poll_read(
            ...
            match self.get_mut().replies.pop() {
                Some(ReadResult::Ok(r)) => {
                    let reply = Vec::<u8>::from(r);
                    buf[..reply.len()].copy_from_slice(&reply);
                    Poll::Ready(Ok(reply.len()))
                }
                Some(ReadResult::TimeoutErr) => {
                    // This error replicates the error returned by usb_bulk when its read times out.
                    let timeout_err = Error::new(ErrorKind::Other, format!("Read error: -110"));
    ...
    impl TestTransport {
        ...
        pub fn push(&mut self, reply: ReadResult) {
