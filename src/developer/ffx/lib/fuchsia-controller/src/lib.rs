...
unsafe fn drop_ptr<T>(ptr: *const T) {
    if ptr != std::ptr::null() {
        let ptr = unsafe { Arc::from_raw(ptr) };
        drop(Arc::try_unwrap(ptr).unwrap_or_else(|_| {
            panic!("dangling reference encountered of {}", std::any::type_name::<T>())
        }));
    }
}
...
mod test {
    ...
    fn testing_lib_context() -> Arc<LibContext> {
        ...
        Arc::new(LibContext::new(ExtBuffer::new(raw, 1024)))
    }
    ...
    fn handle_ready_notification() {
        let lib_ctx = testing_lib_context();
        let fd: RawFd = unsafe { ffx_open_handle_notifier(Arc::as_ptr(&lib_ctx)) };
        assert!(fd > 0);
