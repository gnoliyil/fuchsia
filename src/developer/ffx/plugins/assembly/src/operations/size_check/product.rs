...
pub async fn verify_product_budgets(args: ProductSizeCheckArgs) -> Result<bool> {
    ...
    if let Some(visualization_dir) = args.visualization_dir {
        generate_visualization(&visualization_dir, &package_sizes)?;
        if args.verbose {
...
mod tests {
    use crate::operations::size_check::product::{
        ...
        generate_product_level_diff_output, merge_package_and_product_diffs, PackageBlobSizeInfo,
        PackageLevelDiff, PackageSizeInfo, PackageSizeInfos, ProductLevelDiff,
        TOTAL_BLOBFS_GERRIT_COMPONENT_NAME,
    };
    use anyhow::Result;
