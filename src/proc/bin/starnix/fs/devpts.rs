...
impl FileOps for DevPtmxFile {
    ...
    fn wait_async(
        ...
        Ok(self.terminal.main_wait_async(waiter, events, handler))
    }

    fn cancel_wait(&self, _current_task: &CurrentTask, waiter: &Waiter, key: WaitKey) {
        self.terminal.main_cancel_wait(waiter, key);
    }
...
impl FileOps for DevPtsFile {
    ...
    fn wait_async(
        ...
        Ok(self.terminal.replica_wait_async(waiter, events, handler))
    }

    fn cancel_wait(&self, _current_task: &CurrentTask, waiter: &Waiter, key: WaitKey) {
        self.terminal.replica_cancel_wait(waiter, key);
    }
