...
mod tests {
    ...
    impl<I: TcpTestIpExt, NewIp: TcpTestIpExt> GenericOverIp<NewIp> for TcpCtx<I> {
        type Type = TcpCtx<NewIp>;
    }

    fn handle_timer<I: Ip + TcpTestIpExt>(ctx: &mut TcpCtx<I>, _: &mut (), timer_id: TimerId) {
        I::map_ip(
            (ctx, timer_id),
            |(ctx, timer_id)| {
