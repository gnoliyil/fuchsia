...
// TODO(fxb/84459) Move this to a common location for all BT profiles.
...
pub struct TestProfileServerEndpoints {
    ...
    pub test_server: LocalProfileTestServer,
}

// TODO(fxb/126738) Reomved this when this is moved to a library.
#[allow(unused)]
#[derive(Debug)]
...
pub fn new(
    ...
    let test_server = LocalProfileTestServer::from(stream);
...
/// registration.
pub struct LocalProfileTestServer {
...
impl LocalProfileTestServer {
    ...
    pub fn send_connected(
        ...
        let (near_socket, far_socket) = fidl::Socket::create_stream();
        let near_bt_channel =
            bt::Channel::from_socket(near_socket, 1).expect("failed to create chennel from socket");
        let far_bredr_channel =
            bredr::Channel { socket: Some(far_socket), ..bredr::Channel::default() };
