...
std::vector<sdp::ServiceRecord> GetSPPServiceRecord() {
  ...
  record.AddProtocolDescriptor(sdp::ServiceRecord::kPrimaryProtocolList,
                               sdp::protocol::kL2CAP,
                               sdp::DataElement());
  record.AddProtocolDescriptor(sdp::ServiceRecord::kPrimaryProtocolList,
                               sdp::protocol::kRFCOMM,
...
TEST_F(FakeSdpServerTest, SuccessfulSearch) {
  ...
  sdp::Server::RegistrationHandle spp_handle =
      sdp_server.server()->RegisterService(
          GetSPPServiceRecord(), kChannelParams, NopConnectCallback);
  EXPECT_TRUE(spp_handle);
  sdp::Server::RegistrationHandle a2dp_handle =
      sdp_server.server()->RegisterService(
...
TEST_F(FakeSdpServerTest, ErrorIfTooSmall) {
  ...
  const auto kRspTooSmall =
      SdpErrorResponse(0x1001, sdp::ErrorCode::kInvalidSize);
