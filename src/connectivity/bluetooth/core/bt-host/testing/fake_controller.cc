...
void FakeController::SendCommand(pw::span<const std::byte> command) {
  ...
  heap_dispatcher().Post([self = GetWeakPtr(), command = DynamicByteBuffer(BufferView(command))](
...
void FakeController::ConnectLowEnergy(const DeviceAddress& addr,
  ...
  heap_dispatcher().Post([addr, role, this](pw::async::Context /*ctx*/, pw::Status status) {
    if (!status.ok()) {
      return;
    }
...
void FakeController::L2CAPConnectionParameterUpdate(
  ...
  heap_dispatcher().Post([addr, params, this](pw::async::Context /*ctx*/, pw::Status status) {
    if (!status.ok()) {
      return;
    }
    FakePeer* peer = FindPeer(addr);
...
void FakeController::Disconnect(const DeviceAddress& addr,
  ...
  heap_dispatcher().Post([this, addr, reason](pw::async::Context /*ctx*/, pw::Status status) {
    if (!status.ok()) {
      return;
    }
    FakePeer* peer = FindPeer(addr);
...
void FakeController::OnCreateConnectionCommandReceived(
  ...
  bredr_connect_rsp_task_.Cancel();
  bredr_connect_rsp_task_.set_function(
      [response, peer, this](pw::async::Context /*ctx*/, pw::Status status) mutable {
        if (!status.ok()) {
          return;
        }
        bredr_connect_pending_ = false;
...
void FakeController::OnLECreateConnectionCommandReceived(
  ...
  le_connect_rsp_task_.set_function(
      [packet, address = peer_address, this](pw::async::Context /*ctx*/, pw::Status status) {
...
void FakeController::OnReset() {
  // TODO(https://fxbug.dev/78955): actually do some resetting of stuff here
...
void FakeController::OnLESetScanResponseData(
  ...
  legacy_advertising_state_.scan_rsp_length = params.scan_response_data_length().Read();
...
void FakeController::OnLESetAdvertisingData(
  ...
  legacy_advertising_state_.data_length = params.advertising_data_length().Read();
  ...
  if (params.advertising_data_length().Read() == 0) {
    ...
    std::memcpy(legacy_advertising_state_.data, params.advertising_data().BackingStorage().data(),
...
void FakeController::HandleReceivedCommandPacket(
  ...
  // TODO(https://fxbug.dev/937): Validate size of payload to be the correct length below.
  switch (opcode) {
...
void FakeController::HandleReceivedCommandPacket(const hci::EmbossCommandPacket& command_packet) {
  ...
  switch (opcode) {
    ...
    case hci_spec::kLESetAdvertisingData: {
      const auto& params =
          command_packet.view<pw::bluetooth::emboss::LESetAdvertisingDataCommandView>();
