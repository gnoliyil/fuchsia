...
void FakeController::SendSingleAdvertisingReport(const FakePeer& peer) {
  ...
  bool need_scan_rsp = (le_scan_state().scan_type ==
                        pw::bluetooth::emboss::LEScanType::ACTIVE) &&
                       peer.scannable();
...
void FakeController::OnLESetScanEnable(
  ...
  le_scan_state_.enabled = (params.le_scan_enable().Read() ==
                            pw::bluetooth::emboss::GenericEnableParam::ENABLE);
  le_scan_state_.filter_duplicates =
      (params.filter_duplicates().Read() ==
       pw::bluetooth::emboss::GenericEnableParam::ENABLE);
...
void FakeController::OnLESetAdvertisingEnable(
  ...
  // TODO(https://fxbug.dev/81444): if own address type is random, check that a
...
void FakeController::OnLESetExtendedAdvertisingParameters(
  ...
  // TODO(https://fxbug.dev/80049): Core spec Volume 4, Part E, Section 7.8.53:
...
void FakeController::OnLESetExtendedAdvertisingEnable(
  ...
  for (uint8_t i = 0; i < num_sets; i++) {
    ...
    // TODO(https://fxbug.dev/81444): if own address type is random, check that
...
void FakeController::HandleReceivedCommandPacket(
  ...
  // TODO(https://fxbug.dev/937): Validate size of payload to be the correct
  // length below.
  switch (opcode) {
