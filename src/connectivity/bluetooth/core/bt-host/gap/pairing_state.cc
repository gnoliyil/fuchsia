...
void PairingState::InitiatePairing(
  ...
  // TODO(https://fxbug.dev/132667): Reject pairing if peer/local device don't
  ...
  if (state() == State::kIdle) {
    ...
    // TODO(https://fxbug.dev/42403): If there is no pairing delegate set AND
    ...
    // `send_auth_request`.
    //
    // TODO(https://fxbug.dev/55770): If current IO capabilities would make
...
void PairingState::OnUserConfirmationRequest(uint32_t numeric_value,
  ...
  // TODO(https://fxbug.dev/37447): Reject pairing if pairing delegate went
  // away.
...
void PairingState::OnSimplePairingComplete(
  ...
  if (const fit::result result = ToResult(status_code);
      is_pairing() && bt_is_error(result,
                                  ...
                                  handle(),
                                  bt_str(peer_id()))) {
    // TODO(https://fxbug.dev/37447): Checking pairing_delegate() for reset like
...
void PairingState::OnLinkKeyNotification(
  ...
  // TODO(https://fxbug.dev/36360): We assume the controller is never in pairing
  ...
  // TODO(https://fxbug.dev/132673): When in SC Only mode, all services require
...
std::vector<fit::closure> PairingState::CompletePairingRequests(
  ...
  // satisfied.
  // TODO(https://fxbug.dev/1249): Only notify failure to callbacks of requests
