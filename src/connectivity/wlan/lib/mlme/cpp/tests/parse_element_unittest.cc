...
TEST(ParseElement, SupportedRates) {
  ...
  std::optional<cpp20::span<const SupportedRate>> rates = ParseSupportedRates(raw_body);
  ASSERT_TRUE(rates);
...
TEST(ParseElement, SupportedRatesEmpty) {
  std::optional<cpp20::span<const SupportedRate>> rates = ParseSupportedRates({});
  ASSERT_FALSE(rates);
}
...
TEST(ParseElement, ExtendedSupportedRates) {
  const uint8_t raw_body[] = {10, 20, 30, 40, 50, 60, 70, 80, 90};
  std::optional<cpp20::span<const SupportedRate>> rates = ParseExtendedSupportedRates(raw_body);
  ASSERT_TRUE(rates);
  EXPECT_EQ(raw_body, reinterpret_cast<const uint8_t*>(rates->data()));
...
TEST(ParseElement, ExtendedSupportedRatesEmpty) {
  std::optional<cpp20::span<const SupportedRate>> rates = ParseExtendedSupportedRates({});
  ASSERT_FALSE(rates);
}
...
TEST(ParseElement, MeshId) {
  const uint8_t raw_body[] = {'f', 'o', 'o'};
  std::optional<cpp20::span<const uint8_t>> mesh_id = ParseMeshId(raw_body);
  ASSERT_TRUE(mesh_id);
