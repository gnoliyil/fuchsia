...
def main():
    params = argparse.ArgumentParser(
        description="Verify that non-eng build types have a specified structured configuration policy."
    )
    ...
    with open(args.product_config, "r") as product_config:
        ...
        if build_type != "eng":
            if (
                args.structured_config_policy is None
                or not args.structured_config_policy.is_file()
            ):
                print(
