...
use crate::blob::BlobDirectoryBlobSetBuilder;
...
impl ProductBundleRepositoryBlobs {
    ...
    fn blob_set(&self) -> Result<ProductBundleRepositoryBlobSet, BlobDirectoryBlobSetBuilderError> {
        let blob_set = BlobDirectoryBlobSetBuilder::new().directory(self.directory()).build()?;
...
mod tests {
    ...
    fn utf8_path_buf<P: AsRef<Path>>(path: P) -> Utf8PathBuf {
        Utf8PathBuf::from_path_buf(path.as_ref().to_path_buf()).unwrap()
    }

    fn utf8_path(path: &Path) -> &Utf8Path {
        Utf8Path::from_path(path).unwrap()
    }

    const V2_SDK_A_PRODUCT_BUNDLE_REPOSITORY_NAME: &str = "test.fuchsia.com";
    const V2_SDK_A_PRODUCT_BUNDLE_REPOSITORY_METADATA_PATH: &str = "test_metadata";
    const V2_SDK_A_PRODUCT_BUNDLE_REPOSITORY_BLOBS_PATH: &str = "test_blobs";
