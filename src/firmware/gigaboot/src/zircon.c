...
efi_status generate_efi_memory_attributes_table_item(void* ramdisk, const size_t ramdisk_size,
  ...
  generated_header->number_of_entries = item_count;
...
int boot_zircon(efi_handle img, efi_system_table* sys, void* image, size_t isz, void* ramdisk,
  ...
  for (int attempts = 0;; attempts++) {
    ...
    r = generate_efi_memory_attributes_table_item(ramdisk, rsz, sys, scratch, msize, dsize);
    if (result != EFI_SUCCESS) {
      printf("failed to generate EFI memory attributes table: %s", xefi_strerror(r));
