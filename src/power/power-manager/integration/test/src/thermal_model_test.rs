...
// Thermal policy relies on real timestamps for temperature error calculation. A large
// FAKE_TIME_SCALE value can disrupt timestamps due to IPC time delays. Prior to adjusting
// FAKE_TIME_SCALE, ensure timestamps remain unaffected.
const FAKE_TIME_SCALE: u32 = 20;
...
impl Simulator {
        ...
        fake_clock_control
            .resume_with_increments(
                ...
                &ftesting::Increment::Determined(
                    fuchsia_zircon::Duration::from_millis(FAKE_TIME_SCALE.into()).into_nanos(),
                ),
            )
            .await
    ...
    async fn step_fake_clock(&self) {
        ...
        self.fake_clock_control.pause().await.expect("failed to pause fake time: FIDL error");
...
async fn test_average_temperature() {
    ...
    // Warm up for 10 minutes of simulated time.
    thermal_test_simulator.iterate_n_times(600).await;

    // Calculate the average CPU temperature over the next 10 iterations, and ensure that it's
