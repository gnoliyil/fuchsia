...
""".format(
    path=COMPAT_GUIDE_TOC_ENTRY["path"]
)
...
def binding_title(binding: str) -> str:
    ...
    return (
        binding.upper() if binding in [HLCPP, LLCPP] else binding.capitalize()
    )
...
def write_docs(test_root: Path, test: CompatTest):
    ...
    doc_path = (
        Path(os.environ["FUCHSIA_DIR"]) / DOC_DIR / to_filename(test_root)
    )
