...
def reverse_fidl_steps(
    fidl: FidlDefs, max_step_num: int
    ...
    for old_name, new_name, instructions in zip(
        reversed(refs), reversed_refs, instructions
    ):
...
def reverse_src_steps(
    transition: Steps,
    old_to_new_fidl: Dict[FidlRef, FidlRef],
    max_step_num: int,
) -> (Steps, Dict[str, str]):
    ...
    all_steps = (
        [transition.starting_fidl, transition.starting_src]
        if isinstance(transition.steps[0], FidlStep)
        else [transition.starting_src, transition.starting_fidl]
    )
    all_steps.extend(
        [
...
def reverse_step_nums(nums: List[int], max_step_num: int) -> List[int]:
    assert (
        nums[0] == 0
    ), "every sequence of steps needs to have an initial state"
