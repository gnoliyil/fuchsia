...
void RunRestrictedMode(zx_vaddr_t cs_addr, zx_vaddr_t stack_addr, zx_status_t* result) {
...
TEST(RestrictedModeUnified, Basic) {
  ...
  std::thread restricted_thread(RunRestrictedMode, cs_addr, stack_addr, &result);
