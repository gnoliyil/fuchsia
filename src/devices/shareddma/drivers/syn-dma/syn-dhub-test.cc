...
TEST(SynDhubTest, ConstructForChannel0) {
  ddk_mock::MockMmioRegRegion region(4, as370::kAudioDhubSize / 4);

  // Stop and clear FIFO for cmd and data.
  region[0x1'0a04].ExpectWrite(0x0000'0000);
  region[0x1'0a08].ExpectWrite(0x0000'0001);
  region[0x1'0a14].ExpectWrite(0x0000'0000);
  region[0x1'0a18].ExpectWrite(0x0000'0001);

  // Stop and configure channel.
  region[0x1'0d18].ExpectWrite(0x0000'0000);  // Stop.
  region[0x1'0d00].ExpectWrite(0x0000'0004);  // MTU = 2 ^ 4 x 8 = 128.

  // FIFO cmd configure and start.
  region[0x1'0a00].ExpectWrite(0x0000'0000);  // Base = 0.
  region[0x1'0600].ExpectWrite(0x0000'0004);  // Cell depth = 4.
  region[0x1'0a04].ExpectWrite(0x0000'0001);  // Start.

  // FIFO data configure and start.
  region[0x1'0a10].ExpectWrite(0x0000'0020);  // Base = 32.
  region[0x1'0618].ExpectWrite(0x0000'003c);  // Cell depth = 60.
  region[0x1'0a14].ExpectWrite(0x0000'0001);  // Start.

  // Channel configure and start.
  region[0x1'0d18].ExpectWrite(0x0000'0001);  // Start.
  region[0x1'0100].ExpectWrite(0x0000'0001);  // Cell depth = 1.

  // interrupt setup.
  region[0x1'040c].ExpectRead(0xffff'ffff).ExpectWrite(0xffff'ffff);  // Clear.
  region[0x1'0104].ExpectWrite(0x0000'0002);                          // Enable "full" interrupt.
...
TEST(SynDhubTest, EnableChannel0) {
  ...
  region[0x1'0a04].ExpectWrite(0x0000'0000);  // Stop FIFO cmd queue.
  region[0x1'0d18].ExpectWrite(0x0000'0000);  // Stop channel.
  region[0x1'0d1c].ExpectWrite(0x0000'0001);  // Clear channel.
  region[0x1'0f40].ExpectRead(0x0000'0000);   // Not busy.
  region[0x1'0f44].ExpectRead(0x0000'0000);   // Not pending.

  // Stop and clear FIFO for cmd and data.
  region[0x1'0a04].ExpectWrite(0x0000'0000);
  region[0x1'0a08].ExpectWrite(0x0000'0001);
  region[0x1'0c00].ExpectRead(0x0000'0000);  // FIFO not busy.
  region[0x1'0a14].ExpectWrite(0x0000'0000);
  region[0x1'0a18].ExpectWrite(0x0000'0001);
  region[0x1'0c00].ExpectRead(0x0000'0000);  // FIFO not busy.

  region[0x1'0d18].ExpectWrite(0x0000'0001);  // Start channel.
  region[0x1'0a04].ExpectWrite(0x0000'0001);  // Start cmd queue.
  region[0x1'0a14].ExpectWrite(0x0000'0001);  // Start data queue.
...
TEST(SynDhubTest, EnableChannel10) {
  ...
  region[0x1'0b44].ExpectWrite(0x0000'0000);  // Stop FIFO cmd queue.
  region[0x1'0e80].ExpectWrite(0x0000'0000);  // Stop channel.
  region[0x1'0e84].ExpectWrite(0x0000'0001);  // Clear channel.
  region[0x1'0f40].ExpectRead(0x0000'0000);   // Not busy.
  region[0x1'0f44].ExpectRead(0x0000'0000);   // Not pending.

  // Stop and clear FIFO for cmd and data.
  region[0x1'0b44].ExpectWrite(0x0000'0000);
  region[0x1'0b48].ExpectWrite(0x0000'0001);
  region[0x1'0c00].ExpectRead(0x0000'0000);  // FIFO not busy.
  region[0x1'0b54].ExpectWrite(0x0000'0000);
  region[0x1'0b58].ExpectWrite(0x0000'0001);
  region[0x1'0c00].ExpectRead(0x0000'0000);  // FIFO not busy.

  region[0x1'0e80].ExpectWrite(0x0000'0001);  // Start channel.
  region[0x1'0b44].ExpectWrite(0x0000'0001);  // Start cmd queue.
  region[0x1'0b54].ExpectWrite(0x0000'0001);  // Start data queue.
...
TEST(SynDhubTest, StartDmaForChannel0) {
  ...
  region[0x1'0500].ExpectRead(0x0000'0000);   // Ptr to use.
  region[0x0'0000].ExpectWrite(address);      // Address at the ptr location.
  region[0x0'0004].ExpectWrite(0x1001'0040);  // Size = 60 (in MTUs).
  region[0x1'0900].ExpectWrite(0x0000'0100);  // Push cmd id 0.
