...
class TargetOneDriver : public fdf::DriverBase {
 public:
  TargetOneDriver(fdf::DriverStartArgs start_args,
      ...
      : fdf::DriverBase("target_1_replaced", std::move(start_args), std::move(driver_dispatcher)) {}

  zx::result<> Start() override {
    node_client_.Bind(std::move(node()));

    zx::result result =
        helpers::AddChild(logger(), "Z", node_client_, bindlib::TEST_BIND_PROPERTY_LEAF);
    if (result.is_error()) {
...
FUCHSIA_DRIVER_EXPORT(TargetOneDriver);
