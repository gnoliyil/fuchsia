...
zx_status_t TestRoot::Bind(const char* name, cpp20::span<const zx_device_prop_t> props) {
  ...
  async::TaskClosure init_task([&] {
    ...
    auto serve_status = outgoing_->AddUnmanagedProtocol<fuchsia_composite_test::Device>(
        bindings_.CreateHandler(&this->server_, loop_.dispatcher(), fidl::kIgnoreBindingClosure));
