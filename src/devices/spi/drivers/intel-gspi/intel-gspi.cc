...
void GspiDevice::IrqThread() {
  while (true) {
    ...
    // TODO(https://fxbug.dev/42157505): pay attention to errors and deal with them appropriately.
    if (reg.rfs()) {
  ...
  while (transaction_size > 0) {
    ...
    if (!irq_.is_valid()) {
      ...
      if (return_value != ZX_OK) {
        ...
        // TODO(https://fxbug.dev/42157505): what do we do here? reset the controller?
        break;
      }
    } else {
...
zx_status_t GspiDevice::ValidateChildConfig(Con1Reg& con1) {
  ...
  for (auto& chan : channels) {
    // TODO(https://fxbug.dev/42157505): we should configure clocks, and also track each child. That way we
