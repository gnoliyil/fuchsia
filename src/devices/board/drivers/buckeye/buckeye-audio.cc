...
#ifdef TEST_CODEC
#include "src/devices/board/drivers/buckeye/audio-tas5707-stereo-bind.h"
#include "src/devices/board/drivers/buckeye/tdm-i2s-test-codec-bind.h"
#else
#include "src/devices/board/drivers/buckeye/tdm-i2s-bind.h"
#endif
...
zx_status_t Buckeye::AudioInit() {
  ...
  // Buckeye - High Voltage AMP EN
  // Low then High
  gpio_impl_.SetAltFunction(GPIO_SOC_AMP_EN_H, 0);  // RESET
  gpio_impl_.ConfigOut(GPIO_SOC_AMP_EN_H, 0);
  zx::nanosleep(zx::deadline_after(zx::msec(1)));
  gpio_impl_.ConfigOut(GPIO_SOC_AMP_EN_H, 1);
  zx::nanosleep(zx::deadline_after(zx::msec(15)));
  gpio_impl_.SetDriveStrength(GPIO_SOC_AMP_EN_H, 2500, nullptr);

  // Enable amps
  gpio_impl_.SetAltFunction(GPIO_AMP_EN, 0);
  gpio_impl_.ConfigOut(GPIO_AMP_EN, 1);
  ...
  gpio_impl_.SetAltFunction(A5_GPIOC(6),
                            A5_GPIOC_6_TDMB_D5_FN);  // OUT1
  gpio_impl_.SetDriveStrength(A5_GPIOC(6), ua, nullptr);

  gpio_impl_.SetAltFunction(A5_GPIOC(5),
                            A5_GPIOC_5_TDMB_D4_FN);  // IN1
  ...
  snprintf(metadata.product_name, sizeof(metadata.product_name), "buckeye");
  ...
  metadata.dpad_sel[0] = metadata::AmlTdmDatPad::TDM_D5;  // lane0 <-> TDM_D5(A5_GPIOC_6)
...
#else
  metadata.dai.type = metadata::DaiType::Tdm2;
  metadata.dai.sclk_on_raising = false;
  metadata.dai.bits_per_sample = 16;
  metadata.dai.bits_per_slot = 32;
  metadata.dai.number_of_channels = 8;
  metadata.ring_buffer.number_of_channels = 4;
  metadata.swaps = 0x3210;
  ...
  fpbus::Node tdm_dev;
  char name[32];
  snprintf(name, sizeof(name), "buckeye-i2s-audio-out");
  tdm_dev.name() = name;
...

  {
    ...
    fpbus::Node tdm_dev;
    char name[32];
    snprintf(name, sizeof(name), "buckeye-i2s-audio-in");
    tdm_dev.name() = name;
    tdm_dev.vid() = PDEV_VID_AMLOGIC;
    tdm_dev.pid() = PDEV_PID_AMLOGIC_A5;
    tdm_dev.did() = PDEV_DID_AMLOGIC_TDM;
...

  {
    ...
    gpio_impl_.SetAltFunction(A5_GPIOH(3), A5_GPIOH_3_PDMA_DIN_3_FN);
    gpio_impl_.SetAltFunction(A5_GPIOH(4), A5_GPIOH_4_PDMA_DIN_2_FN);

    metadata::AmlPdmConfig metadata = {};
    snprintf(metadata.manufacturer, sizeof(metadata.manufacturer), "Amlogic");
    snprintf(metadata.product_name, sizeof(metadata.product_name), "buckeye");
    metadata.number_of_channels = 8;
    metadata.version = metadata::AmlVersion::kA5;
    ...
    fpbus::Node pdm_dev;
    char pdm_name[32];
    snprintf(pdm_name, sizeof(pdm_name), "buckeye-pdm-audio-in");
