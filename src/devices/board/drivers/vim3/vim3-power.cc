...
constexpr device_fragment_t power_domain_core_fragments[] = {
...
zx_status_t AddBigCore(fdf::WireSyncClient<fuchsia_hardware_platform_bus::PlatformBus>& pbus) {
  ...
  auto fragments = platform_bus_composite::MakeFidlFragment(fidl_arena, power_domain_core_fragments,
                                                            std::size(power_domain_core_fragments));

  fdf::WireUnownedResult big_core_result =
      pbus.buffer(big_core_arena)
          ->AddCompositeImplicitPbusFragment(fidl::ToWire(fidl_arena, big_core_dev), fragments,
                                             "power-impl");
  if (!big_core_result.ok() || big_core_result.value().is_error()) {
    zxlogf(ERROR, "AddCompositeImplicitPbusFragment for big core failed, error = %s",
...
zx_status_t AddLittleCore(fdf::WireSyncClient<fuchsia_hardware_platform_bus::PlatformBus>& pbus) {
  ...
  fdf::WireUnownedResult little_core_result =
      pbus.buffer(little_core_arena)
          ->AddCompositeImplicitPbusFragment(fidl::ToWire(fidl_arena, little_core_dev), fragments,
                                             "power-impl");
  if (!little_core_result.ok() || little_core_result.value().is_error()) {
    zxlogf(ERROR, "AddCompositeImplicitPbusFragment for little core failed, error = %s",
