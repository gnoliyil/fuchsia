...
class AmlUsbCrgPhyTest : public zxtest::Test {
 public:
  AmlUsbCrgPhyTest() : root_device_(std::make_shared<zx_device_t>()) {
...
TEST_F(AmlUsbCrgPhyTest, SetMode) {
  ...
  auto root_ctx = static_cast<AmlUsbCrgPhy*>(event.device_ctx);
  ...
  ASSERT_EQ(root_ctx->mode(), AmlUsbCrgPhy::UsbMode::HOST);
  ...
  ASSERT_EQ(root_ctx->mode(), AmlUsbCrgPhy::UsbMode::PERIPHERAL);
