...
__EXPORT zx_handle_t get_ioport_resource(zx_device_t* dev) {
  DEBUG_ASSERT_VALID_DEVICE(dev);
  return internal::ContextForApi()->ioport_resource().get();
}

__EXPORT zx_handle_t get_iommu_resource(zx_device_t* dev) {
  DEBUG_ASSERT_VALID_DEVICE(dev);
  return internal::ContextForApi()->iommu_resource().get();
}

__EXPORT zx_handle_t get_framebuffer_resource(zx_device_t* dev) {
  DEBUG_ASSERT_VALID_DEVICE(dev);
  return internal::ContextForApi()->framebuffer_resource().get();
}
...
__EXPORT zx_handle_t get_irq_resource(zx_device_t* dev) {
  DEBUG_ASSERT_VALID_DEVICE(dev);
...
__EXPORT zx_handle_t get_smc_resource(zx_device_t* dev) {
  DEBUG_ASSERT_VALID_DEVICE(dev);
  return internal::ContextForApi()->smc_resource().get();
}
