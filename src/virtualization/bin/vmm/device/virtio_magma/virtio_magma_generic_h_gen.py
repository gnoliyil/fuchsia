...
def generate_generic_method(method, is_internal):
    ...
    ret += " {\n"
    ret += (
        '    TRACE_DURATION("machina", "VirtioMagmaGeneric::Handle_'
        + name
        + '");\n'
    )
    ret += (
        "    FX_DCHECK(request->hdr.type == VIRTIO_MAGMA_CMD_"
        + name.upper()
        + ");\n"
    )
    invocation_args = ""
    ...
    for argument in method["arguments"]:
        ...
        if is_response_argument(argument):
            ...
            invocation_args += (
                "(" + argument["type"] + ")request->" + argument["name"] + ",\n"
            )
...
"""
    for export in magma["exports"] + magma["virtmagma-internal"]:
        ...
        ret += (
            "        auto request = *reinterpret_cast<const virtio_magma_"
            + name
            + "_ctrl_t*>(request_desc.addr);\n"
        )
        ...
        ret += "        uint32_t used;\n"
        ret += (
            "        zx_status_t status = Handle_"
            + name
            + "(&request_desc, &response_desc, &used);\n"
        )
