...
pub struct ComponentSet {
    components: Mutex<HashMap<Id, Weak<Mutex<ElfComponent>>>>,
}
...
impl ComponentSet {
    ...
    pub async fn add(self: Arc<Self>, component: Arc<Mutex<ElfComponent>>) {
        let mut components = self.components.lock().await;
        let id = Id::new();
        let component_set = Arc::downgrade(&self.clone());
        component.lock().await.set_on_drop(move || {
        ...
        components.insert(id, Arc::downgrade(&component));
    }

    /// Invokes `visitor` over all [`ElfComponent`] objects that are currently
    /// running. Note that this is fundamentally racy as a component could be
    /// stopping imminently during or after the visit.
    ///
    /// TODO(fxbug.dev/307580082): Use this soon in `fuchsia.memory.report` server.
    #[allow(dead_code)]
    pub async fn visit(self: Arc<Self>, visitor: &dyn Fn(&ElfComponent)) {
...
mod tests {
    ...
    fn test_add_remove_component() {
        ...
        let fake_component = make_fake_component();
        let mut fut = components.clone().add(fake_component.clone()).boxed();
        assert!(exec.run_until_stalled(&mut fut).is_ready());
    ...
    fn make_fake_component() -> Arc<Mutex<ElfComponent>> {
        ...
        let fake_component = Arc::new(Mutex::new(ElfComponent::new(
            runtime_dir,
            job,
            process,
            lifecycle_channel,
