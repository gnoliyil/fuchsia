...
async fn main() {
    ...
    let event_stream_2 = EventStream::open_at_path("/events/capability_requested_1").await.unwrap();
    handle_capability_requested(event_stream, "0").await;
    handle_capability_requested(event_stream_2, "1").await;
}

async fn handle_capability_requested(mut event_stream: EventStream, response_string: &str) {
    ...
    if let Some(EventPayload::CapabilityRequested(payload)) = event.payload {
        ...
        match request {
            ftest::TriggerRequest::Run { responder } => {
                responder.send(response_string).unwrap();
            }
        }
    } else {
