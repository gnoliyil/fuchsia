...
ComponentControllerBase::GetDir(std::string path) {
  ...
  const fuchsia::io::OpenFlags flags =
      fuchsia::io::OpenFlags::DESCRIBE | fuchsia::io::OpenFlags::RIGHT_READABLE;
  exported_dir_->Open(flags, {} /* mode */, path, diagnostics_dir_node.NewRequest());
