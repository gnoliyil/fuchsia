...
mod test {
    ...
    async fn report_multiple_suites() {
        ...
        let (shell_reporter, output) = ShellReporter::new_expose_writer_for_test();
        let run_reporter = RunReporter::new(shell_reporter);
        run_reporter.set_expected_suites(NUM_SUITES);
        run_reporter.started(Timestamp::Unknown).expect("run started");
        ...
        for suite_number in 0..4 {
            ...
            let case_reporter = suite_reporter.new_case("case-1", &CaseId(0)).expect("create case");
    ...
    async fn report_case_skipped() {
        ...
        case_2_reporter.stopped(&ReportedOutcome::Skipped, Timestamp::Unknown).expect("stop case");
