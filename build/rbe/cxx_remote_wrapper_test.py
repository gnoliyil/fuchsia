...
class CxxRemoteActionTests(unittest.TestCase):
    ...
    def test_remote_cross_compile_clang_with_integrated_preprocessing(self):
        ...
        with mock.patch.object(
            ...
            (
                ...
                sep,
                wrapped_command,
            ) = cl_utils.partition_sequence(remote_compile_command, "--")
            self.assertIn(
        ...
        for status in (0, 1):
            with mock.patch.object(
                subprocess, "call", return_value=status
            ) as mock_cpp:
        ...
        remote_status = 3
        with mock.patch.object(
            cxx_remote_wrapper.CxxRemoteAction, "preprocess_locally"
        ) as mock_cpp:
            with mock.patch.object(
                remote_action.RemoteAction,
                "run_with_main_args",
    ...
    def test_remote_cross_compile_clang_with_local_preprocessing(self):
        ...
        with mock.patch.object(
            ...
            with mock.patch.object(
                cxx_remote_wrapper.CxxRemoteAction, "_cleanup"
            ) as mock_cleanup:
                run_status = c.run()
        self.assertEqual(run_status, remote_status)
        mock_remote.assert_called_once()
