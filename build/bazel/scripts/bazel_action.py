...
def is_hexadecimal_string(s: str) -> bool:
    """Return True if inpuy string only contains hexadecimal characters."""
...
def is_likely_content_hash_path(path: str) -> bool:
    """Returns True if file path is likely based on a content hash.
...
''' % '\n  '.join(list(unknown_labels)),
    ...
    if args.depfile:
        ...
        #   that do not exist, for example `@fuchsia_sdk//pkg/fdio/x64` while
        #   the build configuration only supports arm64). The error looks like:
        #
        #   ```
