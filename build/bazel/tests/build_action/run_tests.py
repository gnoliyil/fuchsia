...
def find_fuchsia_dir(from_path=os.getcwd()):
    ...
    while True:
        ...
        assert (
            new_path != path
        ), f"Could not find Fuchsia directory from {from_path}"
        path = new_path
...
def find_ninja_build_dir(fuchsia_dir):
    ...
    if not os.path.exists(fx_build_dir_file):
        ...
        assert (
            len(build_dir) > 0
        ), f"Empty Ninja build directory: {fx_build_dir_file}"
...
def compare_directories(expected_dir, actual_dir):
    ...
    if dcmp.funny_files:
        print(
            f"ERROR: Uncomparable files found in {actual_dir}:", file=sys.stderr
        )
...
"""
    ...
    for name, must_fail in tests:
        ...
        if must_fail:
            ...
            print(
                "ERROR_STDOUT:\n{stdout}\nERROR_STDERR:\n{stderr}\n\nWhen invoking command: {cmd_str}".format(
                    stdout=ret.stdout, stderr=ret.stderr, cmd_str=cmd_str
                )
            )
            cmd_success = False
        else:
