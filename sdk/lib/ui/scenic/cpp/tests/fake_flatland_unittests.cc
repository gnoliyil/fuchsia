...
class FakeFlatlandTest : public ::testing::Test {
 protected:
  ...
  fuchsia::ui::composition::FlatlandPtr ConnectFlatland() {
    EXPECT_TRUE(!fake_flatland_.is_flatland_connected());

    auto flatland_handle = fake_flatland_.ConnectFlatland(flatland_subloop_->dispatcher());
...
TEST_F(FakeFlatlandTest, Present) {
  ...
  on_next_frame_begin_values.set_additional_present_credits(kOnNextFrameAdditionalPresentCredits);
  flatland.events().OnNextFrameBegin =
      [&on_next_frame_additional_present_credits](auto on_next_frame_begin_values) {
  ...
  fake_flatland().FireOnNextFrameBeginEvent(std::move(on_next_frame_begin_values));
  EXPECT_EQ(on_next_frame_additional_present_credits, 0u);
  loop().RunUntilIdle();
  EXPECT_EQ(on_next_frame_additional_present_credits, kOnNextFrameAdditionalPresentCredits);
  ...
  flatland.events().OnFramePresented =
      [&frame_presented_num_presents_allowed](auto frame_presented_info) {
        ...
        frame_presented_num_presents_allowed = frame_presented_info.num_presents_allowed;
        called_once = true;
      };
  fake_flatland().FireOnFramePresentedEvent(fuchsia::scenic::scheduling::FramePresentedInfo{
  ...
  loop().RunUntilIdle();
  EXPECT_EQ(frame_presented_num_presents_allowed, kOnFramePresentedNumPresentsAllowed);
  ...
  fake_flatland().SetPresentHandler([&present_requested_time](auto present_args) {
    static bool called_once = false;
    EXPECT_FALSE(called_once);
