...
using FakeTransformCache = std::unordered_map<const FakeTransform*, std::shared_ptr<FakeTransform>>;
...
std::shared_ptr<FakeContent> CloneFakeContent(const std::shared_ptr<FakeContent>& content) {
  if (content == nullptr) {
    return nullptr;
  }
...
std::shared_ptr<FakeTransform> CloneFakeTransform(const std::shared_ptr<FakeTransform>& transform,
                                                  FakeTransformCache& transform_cache) {
  ...
  auto [emplaced_transform, success] = transform_cache.emplace(
      transform.get(),
      std::make_shared<FakeTransform>(FakeTransform{
          .id = transform->id,
          ...
          .children = CloneFakeTransformVector(transform->children, transform_cache),
...
bool FakeView::operator==(const FakeView& other) const {
  return view_token == other.view_token && view_ref == other.view_ref &&
         view_ref_control == other.view_ref_control && view_ref_focused == other.view_ref_focused &&
...
bool FakeViewport::operator==(const FakeViewport& other) const {
  return id == other.id && viewport_properties == other.viewport_properties &&
         viewport_token == other.viewport_token && child_view_watcher == other.child_view_watcher;
}
...
bool FakeTransform::operator==(const FakeTransform& other) const {
  return id == other.id && translation == other.translation && *clip_bounds == *other.clip_bounds &&
...
bool FakeGraph::operator==(const FakeGraph& other) const {
  return transform_map == other.transform_map && content_map == other.content_map &&
...
FakeGraph FakeGraph::Clone() const {
  ...
  for (const auto& [transform_id, transform] : transform_map) {
    ZX_ASSERT(transform);
    clone.transform_map.emplace(transform_id, CloneFakeTransform(transform, transform_cache));
  }
  ...
  if (root_transform) {
    ...
    ZX_ASSERT(found_transform != transform_cache.end());
