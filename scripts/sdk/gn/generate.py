...
class GNBuilder(Frontend):
    ...
    def __init__(
        self, local_dir, output, archive="", directory="", jiri_manifest=None
    ):
    ...
    def write_additional_files(self):
        ...
        self.update_metadata(
            [
                "build/gn-rules-meta.json",
                "build/gn-build-files-meta.json",
                "readme-meta.json",
            ]
            + [
            ...
            for entry in entries:
                with open(self.dest(entry), "r") as entry_meta:
                    ...
                    metadata["parts"].append(
                        {"meta": entry, "type": self.get_atom_type(new_meta)}
                    )
    ...
    def _install_ffx_tool_atom_files(self, root, collections):
        ...
        for collection, files in collections.items():
            if (
                ...
                or collection == "executable_metadata"
            ):
        ...
        for arch in self.target_arches:
            ...
            if "dist" in binaries:
                ...
                if not library.is_static:
                    library.lib_name = prebuilt_set.dist_path[
                        len("lib/lib") : -len(".so")
                    ]
    ...
    def install_loadable_module_atom(self, atom):
        ...
        for res in resources:
            ...
            # contain this information (https://fxbug.dev/42122865).
...
def main(args_list=None):
    ...
    parser.add_argument(
        "--output-archive-only",
        help="Whether to output the SDK archive and skip installing to a directory",
        default=False,
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--jiri-manifest",
        help='File containing external references in XML format (e.g. ".jiri_root/update_history/latest")',
    )
    if args_list:
...
def run_generator(
    ...
    if tests:
        ...
        copy_tree(
            os.path.join(testdata, "meta"), os.path.join(wrkspc_sdk_dir, "meta")
        )
