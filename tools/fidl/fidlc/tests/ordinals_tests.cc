...
protocol Special {
  ...
  library.ExpectFail(fidlc::ErrGeneratedZeroValueOrdinal);
...
protocol Special {
  ...
  library.ExpectFail(fidlc::ErrDuplicateMethodOrdinal, "example.fidl:7:5");
  ASSERT_COMPILER_DIAGNOSTICS(library);
}
...
protocol Special {
  ...
  library.UseLibraryZx();
  library.ExpectFail(fidlc::ErrDuplicateMethodOrdinal, "example.fidl:8:5");
  ASSERT_COMPILER_DIAGNOSTICS(library);
}
...
TEST(OrdinalsTests, BadClashingOrdinalBadSelector) {
  ...
  library.ExpectFail(fidlc::ErrDuplicateMethodOrdinal, "bad/fi-0081.test.fidl:7:5");
  ASSERT_COMPILER_DIAGNOSTICS(library);
}
...
protocol protocol {
  ...
  const fidlc::Protocol* iface = library.LookupProtocol("protocol");
...
protocol at {
  ...
  const fidlc::Protocol* iface = library.LookupProtocol("at");
  uint64_t actual_hash64 = iface->methods[0].generated_ordinal64->value;
  ASSERT_EQ(actual_hash64, expected_hash64) << "Expected 64bits hash is not correct";
}
...
protocol at {
  ...
  library.ExpectFail(fidlc::ErrNameNotFound, "nonexistent", "not.important");
  ASSERT_COMPILER_DIAGNOSTICS(library);
}
...
TEST(OrdinalsTests, GoodHackToRenameFuchsiaIoToFuchsiaIoOneNoSelector) {
  ...
  library.ExpectFail(fidlc::ErrFuchsiaIoExplicitOrdinals);
  ASSERT_COMPILER_DIAGNOSTICS(library);
}
...
protocol DirectoryAdmin {
  ...
  library.ExpectFail(fidlc::ErrNameNotFound, "Id", "example");
  ASSERT_COMPILER_DIAGNOSTICS(library);
}
