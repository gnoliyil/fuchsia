...
class GerritServer:
    ...
    def query(self, query: List[Tuple[str, str]]) -> List[Change]:
        ...
        return [
            Change.from_json(x)
            for x in gerrit_util.QueryChanges(
...
def should_submit(
    cl: Change, abort_on_unresolved_comments: bool = True
) -> bool:
...
def print_changes(results: List[Change]) -> None:
    ...
    for result in results:
        ...
        print(
            "%s  %s  %s"
            % (
                status.color()("%20s" % status.description()),
                "%-10s" % result.id,
                "%-65s" % shorten(result.subject, 65),
            )
        )
    print()
...
def ensure_changes_submittable(
    ...
    for cl in changes:
        if cl.status != ChangeStatus.MERGED and not should_submit(
            cl, abort_on_unresolved_comments
        ):
            raise SubmitError(
                "CL %d can not be submitted: %s"
                % (
                    cl.id,
                    cl.status.submit_error_description() or "unknown error",
                )
            )
...
"""
    ...
    parser.add_argument(
        ...
        type=int,
        default=0,
        help="number of times to retry a failed submission. " "Defaults to 0.",
    )
    parser.add_argument(
        "-n",
        "--dry-run",
    ...
    parser.add_argument(
        ...
        "--batch",
        action="store_true",
        help="If specified, don" "t prompt before starting " "submit.",
    )
    parser.add_argument(
        "--ignore-comments",
        action="store_true",
...
def main() -> int:
    ...
    if not is_valid_change_id(args.cl):
        print(
            "The argument '%s' does not look like a valid Gerrit Change ID."
            % args.cl
        )
        print()
        print(
            "Please provide either the numeric CL number (such as 12345) or\n"
    ...
    ensure_changes_submittable(
        changes, abort_on_unresolved_comments=not args.ignore_comments
    )
    ...
    # Submit the changes.
    success = submit_changes(
        util.Clock(), server, changes, num_retries=args.num_retries
    )
    if not success:
        return 1
